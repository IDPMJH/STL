...
void save(string_view FileName)
{
	...
	// std::string NameToSave{ "main_upper.cpp" }; // 저장할 파일 이름
	std::ofstream out{ NameToSave, std::ios::app }; // 덧붙이기로 열기
	//std::ofstream out{ NameToSave, std::ios::trunc }; // 덮어쓰기로 열기
	...
	auto utc = std::chrono::system_clock::to_time_t(now);	// UTC 형식으로 변경 - 협정 세계시
